...
def game(LIST):  # Основная функция программы.
    if LIST == 'ENG':  # Если пользователь введ первую команду выполнится этот блок кода.
    ...
    elif LIST == 'RUS':  # Если пользователь ввел вторую команду выполнится этот блок кода.
        ...
        return  # Завершение программы, с случае с третьей командой или ошибки ввода.
    word = str(input('Введите слово на {} языке: \n'.format(
        lang)).upper())  # Запрос ввода слова от пользователя и приведение его в верхний регистр.
    if (word.isalpha() and all(
            letter in alphabeth_check for letter in word)):  # Проверка соответствия выбранного языка с введенным.
        score = sum(LIST.get(letter, 0) for letter in
                    word)  # Подсчет результата, получение значений ключей списка имеющихся во введенном слове.
...
ENG = {'A': 1, 'E': 1, 'I': 1, 'O': 1, 'U': 1, 'L': 1, 'N': 1, 'S': 1, 'T': 1,
       ...
       'V': 4, 'W': 4, 'Y': 4, 'K': 5, 'J': 8, 'X': 8, 'Q': 10,
       'Z': 10}  # Список, хранящий ключи и их значения, для английского языка.
